...
- 컴퓨터 비전에서 가중치를 초기화해서 훈련시키는 대신 이미 훈련되어있는 구조와 가중치를 다운받아서 사용하는 것이 효과적이다.  
- 특히, 내가 풀고자하는 문제에 사용할 데이터가 적을 경우 최종 분류층인 소프트맥스 층만을 바꿔 훈련시키고 나머지 부분은 학습되지 않게 Freeze시킨다.
- 데이터가 많은 경우에는 Freeze되어 있는 층을 줄여 학습시키거나 매우 많을 경우는 처음부터 새롭게 학습시킬 수도 있다.


## Data Augmentation

- 데이터가 부족한 경우 데이터를 불리는 방법
	- Mirroring(수직축 대칭)
	- Random Cropping (데이터의 일부분을 무작위로 잘라서)
	- Rotate(회전)
	- Shearing(비틀기)
	- Color Shifting
		- 이미지의 RGB채널에 각각 다른 수를 더하거나 빼주는 방식
		- 햇빛의 변화에도 Target의 정보는 변하는 것이 아니기 때문에
		
- 이러한 구현은 주로 CPU thread를 통해서 데이터를 불러옴과 동시에 변형을 시켜 Mini-batch를 형성한다.


## State of Computer Vision

- 대부분의 머신러닝 문제는 데이터의 양에 따라서 설계하는 방법이 다르다.
	- 수 많은 라벨링된 데이터가 존재한다면, 사람이 직접 설계하는 비중을 적게하고 알고리즘에 맡긴다.
	- 반면에 데이터가 부족하다면, 직접 적절한 Feature와 네트워크 구조를 설계하여야 한다.
	
- Dataset Competition에서 벤치마킹을 잘하는 몇 가지의 팁 (다만 실제 제품이나 서비스에는 거의 활용하지 않음)
	- Ensemble: 여러개의 신경망을 독립적으로 훈련시킨 후 평균을 내는 것
	- Multi-Crop at Test: 이미지를 여러개로 크롭핑하여 테스트 후 평균을 낸 결과를 얻는 것
